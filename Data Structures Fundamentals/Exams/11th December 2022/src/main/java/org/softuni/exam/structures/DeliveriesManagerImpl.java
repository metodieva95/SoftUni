...
public class DeliveriesManagerImpl implements DeliveriesManager {
    ...
    private Map<String, Integer> packagesByDelivererCount = new LinkedHashMap<>();

    @Override
    public void addDeliverer(Deliverer deliverer) {
        deliverersById.put(deliverer.getId(), deliverer);
        packagesByDelivererCount.put(deliverer.getId(), 0);
    }
    ...
    public void assignPackage(Deliverer deliverer, Package _package) throws IllegalArgumentException {
        ...
        int currentPackagesCount = packagesByDelivererCount.get(deliverer.getId());
        packagesByDelivererCount.put(deliverer.getId(), currentPackagesCount + 1);
    ...
    public Iterable<Deliverer> getDeliverersOrderedByCountOfPackagesThenByName() {
        return deliverersById.values().stream()
                .sorted(Comparator.comparing( (Deliverer d) -> packagesByDelivererCount.get(d.getId())).reversed()
